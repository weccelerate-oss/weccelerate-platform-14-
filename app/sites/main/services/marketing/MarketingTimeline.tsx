...
// 4 steps: row1 = 2+1, row2 = 1+2
const spanMap = ['md:col-span-2', '', '', 'md:col-span-2'];
...
export default function MarketingTimeline() {
  ...
  const steps: StepData[] = stepImages.map((image, i) => {
    const stepKeys = ['step4', 'step1', 'step2', 'step3'];
    const key = stepKeys[i];
    return {
      id: String(i + 1).padStart(2, '0'),
      title: t(`service.marketing.${key}.title`),
      text: t(`service.marketing.${key}.text`),
      highlights: [
        t(`service.marketing.${key}.h1`),
        t(`service.marketing.${key}.h2`),
        t(`service.marketing.${key}.h3`),
        t(`service.marketing.${key}.h4`),
      ],
      image,
      span: spanMap[i],
    };
  ...
  return (
    <section className="relative py-20 sm:py-28 overflow-hidden">
      ...
      <div className="container-corporate relative z-10 mb-12 sm:mb-16 text-center">
        <motion.span initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="inline-block text-[#c8a951] text-sm font-bold uppercase tracking-[0.3em] mb-4">{t('service.marketing.timeline.tag')}</motion.span>
        <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="text-3xl sm:text-4xl md:text-5xl font-bold text-white tracking-tight">{t('service.marketing.timeline.heading')}</motion.h2>
      </div>
