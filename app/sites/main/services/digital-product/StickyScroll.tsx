...
export default function StickyScroll() {
  ...
  const steps: StepData[] = stepImages.map((image, i) => ({
    ...
    title: t(`service.digital.step${i + 1}.title`),
    text: t(`service.digital.step${i + 1}.text`),
    highlights: [
      t(`service.digital.step${i + 1}.h1`),
      t(`service.digital.step${i + 1}.h2`),
      t(`service.digital.step${i + 1}.h3`),
      t(`service.digital.step${i + 1}.h4`),
    ],
    image,
    span: spanMap[i],
  }));
  ...
  return (
    <section className="relative py-20 sm:py-28 overflow-hidden">
      ...
      <div className="container-corporate relative z-10 mb-12 sm:mb-16 text-center">
        <motion.span initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="inline-block text-[#c8a951] text-sm font-bold uppercase tracking-[0.3em] mb-4">{t('service.digital.timeline.tag')}</motion.span>
        <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="text-3xl sm:text-4xl md:text-5xl font-bold text-white tracking-tight">{t('service.digital.timeline.heading')}</motion.h2>
      </div>
