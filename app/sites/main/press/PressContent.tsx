...
/**
 * PressContent — /press page (fully i18n, RTL/LTR aware).
 *
 * All visible copy resolves through useLanguage()/t('press.*'). Per-mention
 * titles and excerpts are keyed by the stable PressMention `id`; dates, URLs,
 * and outlet names remain data from lib/seo/press-catalog.ts.
 */
      ...
      <div className="container mx-auto px-4 py-12 md:py-16 max-w-4xl">
        {!hasPress && (
          <div className="rounded-2xl border border-white/8 bg-white/[0.02] p-8 text-center">
            <p className="text-base md:text-lg text-white/70">{t('press.empty')}</p>
          </div>
        )}
                ...
                {items.map((mention) => {
                  ...
                  const outletName =
                    lang === 'he'
                      ...
                      : outlet?.name ?? mention.outlet;
                  const dateFormatted = new Date(mention.date).toLocaleDateString(dateLocale, {
                    day: 'numeric',
                    month: 'long',
                    year: 'numeric',
                  });
                  const content = (
                    <article className="rounded-2xl border border-white/8 bg-white/[0.02] p-5 transition-all hover:border-[#c8a951]/30 hover:bg-[#c8a951]/[0.04]">
                      ...
                      <h3 className="mb-2 text-lg font-semibold text-white">
                        {t(`press.item.${mention.id}.title`)}
                      </h3>
                      {mention.excerpt && (
                        <p className="text-sm leading-relaxed text-white/60">
                          {t(`press.item.${mention.id}.excerpt`)}
                        </p>
                      )}
                    </article>
                  );
        ...
        <section className="mt-16 relative overflow-hidden rounded-2xl border border-[#c8a951]/30 bg-gradient-to-br from-[#c8a951]/[0.08] via-transparent to-[#c8a951]/[0.04] p-10 text-center">
          ...
          <div className="relative z-10">
            ...
            <h2 className="mb-3 text-2xl md:text-3xl font-bold">{t('press.cta.title')}</h2>
