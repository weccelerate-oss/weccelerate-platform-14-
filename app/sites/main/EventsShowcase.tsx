...
export function EventsShowcase({ events }: { events: EventItem[] }) {
  ...
  const shortDate = formatShortDate(activeEvent.date);
    ...
    <div>
        ...
        <div className="grid lg:grid-cols-2">
          ...
          <div className="relative aspect-[16/10] lg:aspect-auto lg:min-h-[380px]">
            ...
            <button
              onClick={goPrev}
              disabled={!hasPrev}
              className={`absolute top-1/2 right-3 -translate-y-1/2 z-10 w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 ${
                hasPrev
            ...
            <button
              onClick={goNext}
              disabled={!hasNext}
              className={`absolute top-1/2 left-3 -translate-y-1/2 z-10 w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 ${
                hasNext
                  ? 'bg-black/50 backdrop-blur-sm text-white hover:bg-[#c8a951] hover:text-[#070b1e] border border-white/10'
                  : 'bg-black/20 text-white/20 cursor-not-allowed'
              }`}
              aria-label="אירוע הבא"
            >
            ...
            <div>
              ...
              <div className="space-y-3 mb-8">
                ...
                <div className="flex items-center gap-3 text-sm text-white/40">
                  ...
                  <span>{activeEvent.city || 'אונליין'}</span>
                </div>
              </div>
            </div>
      ...
      <div className="flex gap-3 overflow-x-auto snap-x snap-mandatory pb-2 hide-scrollbar -mx-2 px-2">
        {events.map((event, idx) => {
          const sd = formatShortDate(event.date);
          return (
            <button
              key={event.id}
              onClick={() => goTo(idx)}
              className={`snap-center flex-shrink-0 w-[220px] sm:w-[260px] group relative rounded-xl overflow-hidden text-right transition-all duration-400 ${
                idx === activeIndex
                  ? 'ring-2 ring-[#c8a951] ring-offset-2 ring-offset-[#070b1e] scale-[1.02]'
                  : 'opacity-50 hover:opacity-80'
              }`}
              aria-label={`${event.name}${idx === activeIndex ? ' (מוצג כעת)' : ''}`}
            >
              {/* Image */}
