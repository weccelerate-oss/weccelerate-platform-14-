...
async function stressTest() {
  ...
  // Test 0: Warm up connection
  await prisma.user.findFirst();

  // Test 1: Simple user lookups (sequential)
  ...
  // Test 2: Simple user lookups (parallel)
  ...
  // Test 3: Count records across all tables
  console.log('Test 3: Parallel counts (10 tables)');
  ...
  console.log(`  Files: ${counts[6]}, Notifications: ${counts[7]}, Logs: ${counts[8]}, Lessons: ${counts[9]}`);
  console.log(`  Total: ${elapsed3}ms\n`);

  // Test 4: Complex query with relations (parallel)
  console.log('Test 4: 20 parallel complex queries (Project + User + Files)');
  ...
  console.log('  ✓ Cleaned up stress test records\n');

  // Test 6: Connection pool exhaustion test
  console.log('Test 6: 100 parallel queries (testing connection pool)');
  const start6 = Date.now();
  const promises6 = Array(100).fill(0).map(() =>
    prisma.newsUpdate.findMany({ take: 5, where: { isActive: true } })
  );
  ...
  console.log('=== SUMMARY ===');
  console.log(`Sequential user lookup: ${(elapsed1 / 50).toFixed(2)}ms avg`);
  console.log(`Parallel user lookup: ${(elapsed2 / 50).toFixed(2)}ms avg (${(50000 / elapsed2).toFixed(0)} qps)`);
  console.log(`Complex queries: ${(elapsed4 / 20).toFixed(2)}ms avg`);
  console.log(`Insert performance: ${(elapsed5 / 20).toFixed(2)}ms avg`);
  console.log(`100-connection pool: ${(100000 / elapsed6).toFixed(0)} qps\n`);
